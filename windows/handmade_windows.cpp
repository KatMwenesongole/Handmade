...
internal void
io_writeconsole(void* message, u32 message_size)
{
...
internal void os_lockcursor()
{
    // place cursor in the centre.
	...
	POINT cursor_position = { (LONG)(width * 0.5), (LONG)(height * 0.5) };
    ...
    if(cursor_info.flags > 0) { ShowCursor(false); }
}
internal void os_unlockcursor()
{
    // show cursor.
    ...
    if(cursor_info.flags < 1) { ShowCursor(true); }
}
                          ...
                          s8* window_title)
{
    ...
    window_class.hIcon         = LoadIconA(GetModuleHandle(0), MAKEINTRESOURCE(1));
